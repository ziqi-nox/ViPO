...
def retrieve_text(frames, 
    ...
    # probs, idxs = clip.get_predict_score(vid_feat, text_feats_tensor, top=topk)
    probs, idxs = clip.get_predict_score(vid_feat, text_feats_tensor, top=topk)
...
def inference_viclip_reward_score(video_path, 
                  text, 
                  models={'viclip':None, 
                          'tokenizer':None},
                  device=torch.device('cuda')):
    ...
    text_feat = [text_feat_d[t] for t in text]
    text_feats_tensor = torch.cat(text_feat, 0)
    score = clip.get_predict_score(vid_feat, text_feats_tensor)
